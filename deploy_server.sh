...
if [ ! -f "$ENV_FILE" ]; then
    ...
    grep -q "APP_PORT" "$ENV_FILE" || echo "APP_PORT=8020" >> "$ENV_FILE"
...
if [ -d "nginx_hestia_templates" ]; then
    ...
    cp nginx_hestia_templates/django-8020.tpl "$NGINX_PROXY_DIR/"
    cp nginx_hestia_templates/django-8020.stpl "$NGINX_PROXY_DIR/"
    ...
    echo "  → django-8020.tpl"
    echo "  → django-8020.stpl"
fi

echo -e "${GREEN}Deployment Finished Successfully!${NC}"
echo "App reachable at http://127.0.0.1:${APP_PORT:-8020} locally."
echo "Hestia public_html now contains only static assets in $PUBLIC_DIR/static."
echo "Ensure your Hestia Proxy Template matches APP_PORT (for example, use 'django-8020' for port 8020)."
